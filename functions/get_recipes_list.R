...
get_recipes_list <- function(recipes_text_file = RECIPES_TEXT_FILE, 
  ...
  recipes_list <- vector("list", length = length(recipes_text_list))
  
  lapply(seq(recipes_text_list), function(r) {
    recipe <<- recipes_text_list[r]
    recipes_list[[r]]$name <<- extract_recipe_name(recipe)
    recipes_list[[r]]$ingredients <<- extract_recipe_ingredients(recipe)
    recipes_list[[r]]$instructions <<- extract_recipe_instructions(recipe)
    recipes_list[[r]]$tags <<- extract_recipe_tags(recipe) #%>% replace("", NA)
    recipes_list[[r]]$scalable <<- extract_recipe_scalability(recipe)
  })
